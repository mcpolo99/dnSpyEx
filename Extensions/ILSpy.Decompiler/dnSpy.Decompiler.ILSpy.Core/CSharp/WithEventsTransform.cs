	...
	sealed class WithEventsTransform : IAstTransform {
		...
		public void Run(AstNode compilationUnit) {
			...
			foreach (var propDecl in compilationUnit.Descendants.OfType<PropertyDeclaration>()) {
				...
				if (def is null || !withEventsProperties.Contains(def))
					continue;

				var events = ExtractEventSubscriptions(propDecl);
				propertyEvents[def.Name] = events;
				propertiesToReplace.Add((propDecl, def));
			}
		...
		string? FindHandlerMethodName(BlockStatement body, Expression handlerExpr) {
			// The handler expression in += is usually a local variable
			// Trace back to its declaration to find the method name
			if (handlerExpr is IdentifierExpression ident) {
				// Find the variable declaration: Type varName = new DelegateType(this.MethodName);
				foreach (var varDecl in body.Descendants.OfType<VariableDeclarationStatement>()) {
					foreach (var variable in varDecl.Variables) {
						if (variable.Name == ident.Identifier && variable.Initializer is ObjectCreateExpression objCreate) {
							// The argument to the delegate constructor is the method reference
